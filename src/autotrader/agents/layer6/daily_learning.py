...
def _generate_report(state: TradingState, stats: dict) -> str:
...
_Symbols that had high scores but were filtered by governance/risk. Review these manually._

{chr(10).join(f"- {o.get('symbol')}: Score={o.get('score', 0):.1f}" for o in scored[:5]) or "- None identified"}
...
def daily_learning_agent(state: TradingState) -> dict[str, Any]:
    ...
    report_content = _generate_report(state, stats)
